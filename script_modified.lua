...
local spamTarget = (Config.SpamTarget or ""):lower()
...
-- ── SPAM TARGET (selalu terlihat, tidak perlu expand Settings) ──
local SpamFrame = Instance.new("Frame", MainFrame)
SpamFrame.Size = UDim2.new(1, -10, 0, 36)
SpamFrame.Position = UDim2.new(0, 5, 0, 112)
SpamFrame.BackgroundColor3 = THEME.ItemBG
Instance.new("UICorner", SpamFrame).CornerRadius = UDim.new(0, 6)

local SpamIcon = Instance.new("TextLabel", SpamFrame)
SpamIcon.Text = "🎯"
SpamIcon.Font = Enum.Font.Gotham
SpamIcon.TextSize = 15
SpamIcon.Size = UDim2.new(0, 30, 1, 0)
SpamIcon.Position = UDim2.new(0, 8, 0, 0)
SpamIcon.BackgroundTransparency = 1
SpamIcon.TextColor3 = THEME.SubText

local SpamTargetBox = Instance.new("TextBox", SpamFrame)
SpamTargetBox.Size = UDim2.new(1, -78, 1, 0)
SpamTargetBox.Position = UDim2.new(0, 38, 0, 0)
SpamTargetBox.BackgroundTransparency = 1
SpamTargetBox.Font = Enum.Font.Gotham
SpamTargetBox.TextSize = 15
SpamTargetBox.TextColor3 = THEME.Text
SpamTargetBox.PlaceholderText = "Spam target akhiran (mis. tes)"
SpamTargetBox.PlaceholderColor3 = THEME.SubText
SpamTargetBox.Text = spamTarget
SpamTargetBox.TextXAlignment = Enum.TextXAlignment.Left
SpamTargetBox.ClearTextOnFocus = false
SpamTargetBox.TextEditable = true
SpamTargetBox.ClipsDescendants = false

local SpamClearBtn = Instance.new("TextButton", SpamFrame)
SpamClearBtn.Text = "✕"
SpamClearBtn.Font = Enum.Font.GothamBold
SpamClearBtn.TextSize = 16
SpamClearBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
SpamClearBtn.BackgroundTransparency = 1
SpamClearBtn.Size = UDim2.new(0, 36, 1, 0)
SpamClearBtn.Position = UDim2.new(1, -36, 0, 0)

SpamTargetBox:GetPropertyChangedSignal("Text"):Connect(function()
    spamTarget = SpamTargetBox.Text:lower():gsub("[^a-z]", "")
    Config.SpamTarget = spamTarget
    SaveConfig()
    if UpdateList then
        UpdateList(lastDetected, lastRequiredLetter)
    end
...
SpamClearBtn.MouseButton1Click:Connect(function()
    spamTarget = ""
    SpamTargetBox.Text = ""
    Config.SpamTarget = ""
    SaveConfig()
...
ScrollList.Size = UDim2.new(1, -10, 1, -258)
ScrollList.Position = UDim2.new(0, 5, 0, 153)
...
TogglesFrame.Size = UDim2.new(1, 0, 0, 360)
...
local function UpdateLayout()
    if settingsCollapsed then
        ...
        Tween(ScrollList, {Size = UDim2.new(1, -10, 1, -260)})
        TogglesFrame.Visible = false
    else
        Tween(SettingsFrame, {Size = UDim2.new(1, 0, 0, 526), Position = UDim2.new(0, 0, 1, -526)})
        Tween(ScrollList, {Size = UDim2.new(1, -10, 1, -656)})
...
local function GenerateKeyboard()
    ...
    if space:FindFirstChild("TextLabel") then
        space.TextLabel.Text = "SPACE"
    end
end
...
local HumanizeBtn = CreateToggle("Humanize: "..(useHumanization and "ON" or "OFF"), UDim2.new(0, 8, 0, 5), function()
...
local FingerBtn = CreateToggle("10-Finger: "..(useFingerModel and "ON" or "OFF"), UDim2.new(0, 104, 0, 5), function()
...
local KeyboardBtn = CreateToggle("Keyboard: "..(showKeyboard and "ON" or "OFF"), UDim2.new(0, 200, 0, 5), function()
...
local SortBtn = CreateToggle("Sort: "..sortMode, UDim2.new(0, 8, 0, 40), function()
...
SortBtn.Size = UDim2.new(0, 136, 0, 30)

local AutoBtn = CreateToggle("Auto Play: "..(autoPlay and "ON" or "OFF"), UDim2.new(0, 152, 0, 40), function()
...
AutoBtn.Size = UDim2.new(0, 136, 0, 30)

local AutoJoinBtn = CreateToggle("Auto Join: "..(autoJoin and "ON" or "OFF"), UDim2.new(0, 8, 0, 76), function()
...
AutoJoinBtn.Size = UDim2.new(0, 280, 0, 30)
...
local function CreateCheckbox(text, pos, key)
    ...
    check.Visible = Config.AutoJoinSettings[key] or false
...
CreateCheckbox("1v1", UDim2.new(0, 8, 0, 112), "1v1")
CreateCheckbox("4 Player", UDim2.new(0, 104, 0, 112), "4p")
CreateCheckbox("8 Player", UDim2.new(0, 200, 0, 112), "8p")

local BlatantBtn = CreateToggle("Blatant Mode: "..(isBlatant and "ON" or "OFF"), UDim2.new(0, 8, 0, 184), function()
...
BlatantBtn.Size = UDim2.new(0, 136, 0, 30)

local RiskyBtn = CreateToggle("Risky Mistakes: "..(riskyMistakes and "ON" or "OFF"), UDim2.new(0, 152, 0, 184), function()
...
RiskyBtn.Size = UDim2.new(0, 136, 0, 30)
...
ManageWordsBtn.TextSize = 12
ManageWordsBtn.TextColor3 = THEME.Accent
ManageWordsBtn.BackgroundColor3 = THEME.Background
ManageWordsBtn.Size = UDim2.new(0, 280, 0, 32)
ManageWordsBtn.Position = UDim2.new(0, 8, 0, 184)
Instance.new("UICorner", ManageWordsBtn).CornerRadius = UDim.new(0, 6)
...
WordBrowserBtn.Size = UDim2.new(0, 280, 0, 32)
WordBrowserBtn.Position = UDim2.new(0, 8, 0, 222)
Instance.new("UICorner", WordBrowserBtn).CornerRadius = UDim.new(0, 6)
...
ServerBrowserBtn.TextSize = 12
ServerBrowserBtn.TextColor3 = Color3.fromRGB(100, 200, 255)
ServerBrowserBtn.BackgroundColor3 = THEME.Background
ServerBrowserBtn.Size = UDim2.new(0, 280, 0, 32)
ServerBrowserBtn.Position = UDim2.new(0, 8, 0, 260)
Instance.new("UICorner", ServerBrowserBtn).CornerRadius = UDim.new(0, 6)

local CustomWordsFrame = Instance.new("Frame", ScreenGui)
CustomWordsFrame.Name = "CustomWordsFrame"
CustomWordsFrame.Size = UDim2.new(0, 300, 0, 420)
CustomWordsFrame.Position = UDim2.new(0.5, -150, 0.5, -210)
...
CWHeader.Text = "✦ Custom Words"
CWHeader.Font = Enum.Font.GothamBold
CWHeader.TextSize = 15
CWHeader.TextColor3 = THEME.Text
CWHeader.Size = UDim2.new(1, -50, 0, 44)
CWHeader.Position = UDim2.new(0, 15, 0, 0)
CWHeader.BackgroundTransparency = 1
CWHeader.TextXAlignment = Enum.TextXAlignment.Left

local CWCloseBtn = Instance.new("TextButton", CustomWordsFrame)
CWCloseBtn.Text = "✕"
CWCloseBtn.Font = Enum.Font.GothamBold
CWCloseBtn.TextSize = 16
CWCloseBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
CWCloseBtn.Size = UDim2.new(0, 44, 0, 44)
CWCloseBtn.Position = UDim2.new(1, -44, 0, 0)
...
CWSearchBox.TextSize = 14
CWSearchBox.BackgroundColor3 = THEME.ItemBG
CWSearchBox.Size = UDim2.new(1, -20, 0, 36)
CWSearchBox.Position = UDim2.new(0, 10, 0, 48)
Instance.new("UICorner", CWSearchBox).CornerRadius = UDim.new(0, 6)
SetupPhantomBox(CWSearchBox, "🔍 Search words...")

local CWScroll = Instance.new("ScrollingFrame", CustomWordsFrame)
CWScroll.Size = UDim2.new(1, -10, 1, -145)
CWScroll.Position = UDim2.new(0, 5, 0, 90)
CWScroll.BackgroundTransparency = 1
CWScroll.ScrollBarThickness = 3
...
CWListLayout.Padding = UDim.new(0, 3)

local CWAddBox = Instance.new("TextBox", CustomWordsFrame)
CWAddBox.Font = Enum.Font.Gotham
CWAddBox.TextSize = 14
CWAddBox.BackgroundColor3 = THEME.ItemBG
CWAddBox.Size = UDim2.new(1, -80, 0, 40)
CWAddBox.Position = UDim2.new(0, 10, 1, -50)
Instance.new("UICorner", CWAddBox).CornerRadius = UDim.new(0, 6)
...
CWAddBtn.TextSize = 13
CWAddBtn.TextColor3 = Color3.fromRGB(255,255,255)
CWAddBtn.BackgroundColor3 = THEME.Accent
CWAddBtn.Size = UDim2.new(0, 64, 0, 40)
CWAddBtn.Position = UDim2.new(1, -74, 1, -50)
Instance.new("UICorner", CWAddBtn).CornerRadius = UDim.new(0, 6)
...
local function RefreshCustomWords()
    ...
    for i, w in ipairs(list) do
        if query == "" or w:find(query, 1, true) then
            ...
            del.Size = UDim2.new(0, 38, 1, 0)
            del.Position = UDim2.new(1, -38, 0, 0)
    ...
    CWScroll.CanvasSize = UDim2.new(0, 0, 0, shownCount * 41)
end
...
UpdateList = function(detectedText, requiredLetter)
    ...
    if spamTarget ~= "" and #matches > 0 then
        -- Mode Spam Target aktif: pisahkan matches jadi 2 grup
        -- Grup A = kata yang BERAKHIR dengan spamTarget (buat nyerang/nge-spam)
        -- Grup B = kata yang TIDAK berakhir spamTarget (buat jaga-jaga, hindari dead end)
        local groupA, groupB = {}, {}
        local targetLen = #spamTarget

        for _, w in ipairs(matches) do
            local wLower = w:lower()
            if #wLower >= targetLen and wLower:sub(-targetLen) == spamTarget then
                table.insert(groupA, w)
            else
                table.insert(groupB, w)
            end
        end

        -- Prioritas: isi slot dengan groupA dulu (maksimal 2 slot),
        -- sisanya (minimal 1 slot) diisi groupB sebagai kata aman cadangan
        local slotsForA = math.min(#groupA, maxDisplay - 1)
        if slotsForA < 1 and #groupA > 0 then slotsForA = 1 end -- kalau ada groupA, tetap tampilkan minimal 1

        for i = 1, slotsForA do
            table.insert(displayList, groupA[i])
        end

        local remaining = maxDisplay - #displayList
        for i = 1, math.min(remaining, #groupB) do
            table.insert(displayList, groupB[i])
        end

        -- Kalau groupB kosong tapi groupA masih ada sisa, isi sisa slot dengan groupA juga
        if #displayList < maxDisplay then
            for i = slotsForA + 1, #groupA do
    ...
    -- Safety net: kalau entah kenapa displayList kosong padahal matches ada isi, fallback normal
    if #displayList == 0 and #matches > 0 then
        for i = 1, math.min(maxDisplay, #matches) do table.insert(displayList, matches[i]) end
    end
    ...
    if UpdateDangerAlert then UpdateDangerAlert(currentBestMatch) end
    ...
    for i = 1, math.max(#displayList, #ButtonCache) do
        ...
        if w then
            ...
            -- Mode Spam Target: tandai kata target (🎯 hijau) vs kata aman cadangan (🛡 abu)
            if spamTarget ~= "" then
                local wLower = w:lower()
                local targetLen = #spamTarget
                local isTargetWord = #wLower >= targetLen and wLower:sub(-targetLen) == spamTarget
                if isTargetWord then
                    displayText = "🎯 " .. displayText
                    if lbl then lbl.TextColor3 = Color3.fromRGB(255, 180, 80) end
                else
                    displayText = "🛡 " .. displayText
...
-- ── DANGER ALERT (mengambang terpisah, tidak ketutupan saat ngetik) ──
-- Muncul otomatis kalau top match punya solve count rendah (1-2)
local DangerAlert = Instance.new("Frame")
DangerAlert.Name = "DangerAlert"
DangerAlert.Size = UDim2.new(0, 190, 0, 46)
DangerAlert.Position = UDim2.new(0, 12, 0, 80)
DangerAlert.BackgroundColor3 = THEME.Background
DangerAlert.Visible = false
DangerAlert.ZIndex = 150
DangerAlert.Parent = ScreenGui
EnableDragging(DangerAlert)
Instance.new("UICorner", DangerAlert).CornerRadius = UDim.new(0, 8)
local DAStroke = Instance.new("UIStroke", DangerAlert)
DAStroke.Color = Color3.fromRGB(255, 80, 80)
DAStroke.Thickness = 2

local DALabel = Instance.new("TextLabel", DangerAlert)
DALabel.Size = UDim2.new(1, -12, 0, 18)
DALabel.Position = UDim2.new(0, 8, 0, 4)
DALabel.BackgroundTransparency = 1
DALabel.Font = Enum.Font.GothamBold
DALabel.TextSize = 12
DALabel.TextColor3 = Color3.fromRGB(255, 100, 100)
DALabel.TextXAlignment = Enum.TextXAlignment.Left
DALabel.Text = "⚠ LOW SOLVE"

local DAWord = Instance.new("TextLabel", DangerAlert)
DAWord.Size = UDim2.new(1, -12, 0, 20)
DAWord.Position = UDim2.new(0, 8, 0, 22)
DAWord.BackgroundTransparency = 1
DAWord.Font = Enum.Font.GothamBold
DAWord.TextSize = 15
DAWord.TextColor3 = THEME.Text
DAWord.TextXAlignment = Enum.TextXAlignment.Left
DAWord.TextTruncate = Enum.TextTruncate.AtEnd
DAWord.Text = "--"

-- Hitung berapa banyak kata di Buckets yang mulai dengan huruf akhir dari `word`
local function CountSolves(word)
    if not word or #word == 0 then return 999 end
    local lastChar = word:sub(-1)
    local bucket = Buckets[lastChar]
    if not bucket then return 0 end
    local count = 0
    for _, w in ipairs(bucket) do
        if not Blacklist[w] and not UsedWords[w] then
            count = count + 1
            if count > 2 then return count end -- cukup tahu > 2, tidak perlu hitung semua
        end
    end
    return count
end

UpdateDangerAlert = function(topWord)
    if not topWord then
        DangerAlert.Visible = false
        return
    end
    local solves = CountSolves(topWord)
    if solves <= 2 then
        DAWord.Text = topWord
        DALabel.Text = (solves == 0) and "☠ DEAD END" or ("⚠ ONLY " .. solves .. " SOLVE" .. (solves > 1 and "S" or ""))
        DAStroke.Color = (solves == 0) and Color3.fromRGB(255, 40, 40) or Color3.fromRGB(255, 150, 50)
                ...
                if matches then
                    for _, matchFrame in ipairs(matches:GetChildren()) do
                        if (matchFrame:IsA("Frame") or matchFrame:IsA("GuiObject")) and matchFrame.Name ~= "UIListLayout" then
                            ...
                            if title and title:IsA("TextLabel") and title.Text:find("Last Letter") then
                                isLastLetter = true
                            end
                            ...
                            local allowed = true
                            if idx then
                                if idx >= 1 and idx <= 4 then allowed = Config.AutoJoinSettings["1v1"]
                                elseif idx >= 5 and idx <= 8 then allowed = Config.AutoJoinSettings["4p"]
                                elseif idx == 9 then allowed = Config.AutoJoinSettings["8p"]
                                end
                            end
                            ...
                            if joinBtn and joinBtn.Visible and isLastLetter and allowed then
                                ...
                                if (tick() - (JoinDebounce[matchId] or 0)) > 2 then
                                    ...
                                    local clicked = false
                                    if getconnections and joinBtn:IsA("GuiButton") then
...
-- ── FLOATING TOGGLE BUTTON (Mobile-friendly, ganti keyboard shortcut) ──
...
ToggleBtn.MouseButton1Click:Connect(function()
    ...
    Tween(ToggleBtn, {BackgroundColor3 = MainFrame.Visible and THEME.Accent or Color3.fromRGB(80,80,90)}, 0.2)
end)
